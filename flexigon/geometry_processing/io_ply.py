...
def read_ply(filename):
    ...
    if fmt == 'ascii':
        ...
        data["faces"] = torch.from_numpy(data["faces"].to_numpy().astype(np.int32)).to('cuda')
    else:
        with open(filename, 'rb') as ply:
            ply.seek(end_header)
            ...
            if mesh_size:
                ...
                data["faces"] = torch.tensor(np.stack((np.int32(mesh_np["v1"]), np.int32(mesh_np["v2"]), np.int32(mesh_np["v3"])), axis=1), device='cuda')
...
def savePLY(filename='savedPLY.ply', vertices_save=[], indices_save=[], color_save=[]):
    # save ply from vertices [] , faces and texture
    if color_save != [] and color_save.dtype != 'uint8':
        if np.max(color_save) > 1:
            color_save = np.uint8(color_save)
        else:
            color_save = np.uint8(color_save * 255)
    if np.min(indices_save) == 1:
        indices_save -= 1

    indices_save = np.asarray(indices_save, np.int32)
    # write ply
    ...
    with open(filename, "w") as f:
        ...
        f.write("element vertex " + str(len(vertices_save)) + "\n")
        ...
        f.write("element face " + str(np.shape(indices_save)[0]) + "\n")
        f.write("property list uchar int vertex_indices\n")
        f.write("end_header\n")

        if color_save != []:
            for vertex, color in zip(vertices_save, color_save):
                f.write(str(vertex[0]) + " " + str(vertex[1]) + " " + str(vertex[2]) + " ")
                f.write(str(color[0]) + " " + str(color[1]) + " " + str(color[2]) + "\n")
        else:
            for vertex in vertices_save:
                f.write(str(vertex[0]) + " " + str(vertex[1]) + " " + str(vertex[2]) + "\n")

        for face in indices_save:
...
def read_obj_file(file_path):
    ...
    vertices = torch.from_numpy(np.stack(vertices)).to(torch.float32).to('cuda')
    faces = torch.from_numpy(np.stack(faces)).to(torch.float32).to('cuda')
