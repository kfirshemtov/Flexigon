...
import torch
import numpy as np
from flexigon.render_engine.io_ply import read_ply
import imageio
...
def load_scene(filepath):
    ...
    for plugin in root:
        if plugin.tag == "default":
            ...
            elif plugin.attrib["name"] == 'cam_proj_mat':
                scene_params["cam_proj_mat"] = plugin.attrib["value"]
            ...
            if plugin.attrib["type"] == "ply":
                for prop in plugin:
                    if prop.tag == "string" and prop.attrib["name"] == "filename":
                        ...
                        mesh_id = plugin.attrib["id"]

                        # Save the path and the PLY data separately
                        scene_params[f"{mesh_id}_path"] = mesh_path
                        scene_params[mesh_id] = read_ply(mesh_path)
            else:
